...
- Per-floor/level visual differentiation (biome identity, fog, further work on `generateFloorPatterns`) is being handled in a separate effort — don't start on it unless explicitly asked, even if it looks like an obvious next step from the code.
